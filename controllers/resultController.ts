...
export async function addResult(
  ...
  try {
    ...
    if (existingResult) {
      return res.status(400).json({
        message:
          "A result with this encounterUuid and testName already exists.",
      });
    }
...
//get unique result by testname

export async function getUniqueResultByTestName(
  req: Request<{ uuid: string, testname:string }, {}, IAddResult>,
  res: Response
) {
  const uuid = req.params.uuid;
  ...
  try {
    const result = await resultModel.findOne({encounterUuid:uuid, "testDetails.testName":testname})
...
export async function downloadUniqueResultByTestName(
  req: Request<{ uuid: string, testname:string }, {}, IAddResult>,
  res: Response
) {
  const uuid = req.params.uuid;
